...
impl Layer {
    ...
    pub fn forward(&mut self, input: &Array1<f32>, use_activation: bool) -> Array1<f32> {
        ...
        let activated = if use_activation {
            output.mapv(relu)
        } else {
            output
        };
    ...
    pub fn backward(&mut self, grad_output: &Array1<f32>, use_activation: bool) -> Array1<f32> {
        let grad = if use_activation {
            ...
            grad_output * &output.mapv(relu_derivative)
        } else {
            grad_output.clone()
        };
        ...
        let grad_weights = grad.clone().insert_axis(ndarray::Axis(1)).dot(&input.clone().insert_axis(ndarray::Axis(0)));
